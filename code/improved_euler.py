...
class ImprovedEulerMethod(NumericMethod):
    ...
    def calculateY(self, x, y):
        h = self.h
        Y = y + (h/2) * (self.exactSolution.Derivative(x, y) +
         self.exactSolution.Derivative(x + h, y + h * self.exactSolution.Derivative(x, y)))
        return Y

    def calculateGlobalError(self, n0):
        error = np.empty(self.N - n0, dtype=float)
        for i in range(n0,self.N):
            exact=MyOwnSolution(i,self.Y0,self.Y0,self.bigX)
            method = ImprovedEulerMethod(i,self.Y0,self.X0,self.bigX,exact)
